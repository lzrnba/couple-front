  ...
  <div class="container">
    <div class="form_area">
      ...
      <form @submit.prevent="handleResetPassword">
        ...
        <div class="form_group">
          <label class="sub_title" for="newPassword">新密码</label>
          <input 
        ...
        <div>
          ...
          <p><router-link class="link" to="/login">返回登录</router-link></p>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async handleResetPassword() {
      if (!this.form.email || !this.form.code || !this.form.newPassword) {
      ...
      try {
        ...
        if (response.code === 200) {
          this.showNotification('重置成功', '密码重置成功，即将跳转到登录页面', 'success');
          setTimeout(() => {
            this.$router.push('/login');
          }, 2000);
        } else {
          this.showNotification('重置失败', `密码重置失败: ${response.message || '未知错误'}`, 'error');
        }
      } catch (error) {
        console.error('Reset password error:', error);
        this.showNotification('重置错误', '密码重置请求失败，请稍后重试', 'error');
      } finally {
        this.loading = false;
      }
    }
